...
import { skinTreatments } from "../data/services";
import { skinTreatmentDetails } from "../data/skinTreatmentDetails";
import { clinic } from "../data/site";
import Button from "../components/Button";

export default function SkinTreatmentDetail() {
  const { slug } = useParams();
  const treatment = skinTreatments.find((t) => t.slug === slug);
  const detail = skinTreatmentDetails[slug];
        ...
        <div className="container-px mx-auto max-w-7xl py-3">
          <div className="flex items-center justify-between gap-4">
            <nav className="flex items-center gap-1.5 text-sm text-ink-500" aria-label="Breadcrumb">
              ...
              <ChevronRight className="h-3.5 w-3.5 text-ink-300" />
              <Link to="/Skin-treatment" className="hover:text-brand-600 transition-colors">Skin Treatments</Link>
              <ChevronRight className="h-3.5 w-3.5 text-ink-300" />
